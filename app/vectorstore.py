...
def _embed_and_persist(
    ...
    BATCH_SIZE = 100  # smaller batches = safer, easier to debug
    total = len(chunks)

    # Create collection upfront with correct vector size
    ...
    print(f"[vectorstore] ✅ Persisted {total} chunks to Qdrant: {settings.QDRANT_URL}")
...
def get_vectorstore(rebuild: bool = False) -> QdrantVectorStore:
    ...
    if not rebuild and _is_collection_populated(client):
        print(f"[vectorstore] Loading existing Qdrant collection: {COLLECTION}")
        return QdrantVectorStore(
            client=client,
            collection_name=COLLECTION,
            embedding=embeddings,
